    ...
    try {
      ...
      if (data.token) {
        ...
        if (newUser) {
          ...
          let accountsUser = data.user as Record<string, unknown>;
            ...
            if (discordId) {
              ...
              try {
                ...
                if (botToken) {
                  ...
                  if (dUser.ok) {
                    const dData = await dUser.json() as Record<string, unknown>;
  ...
  .get('/:provider/callback', async ({ params, query, cookie }) => {
    ...
    if (provider === 'lastfm') {
      ...
      try {
        ...
        const infoData = await infoResp.json() as Record<string, unknown>;
